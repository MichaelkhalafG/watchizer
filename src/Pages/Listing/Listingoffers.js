...
function Listingoffers() {
    const { language, offers, offersfilters, setOffersFilters } = useContext(MyContext);
    ...
    useEffect(() => {
        ...
        //         offersfilters.categories.includes(product.category_type_id)
        //     );
        // }
        // if (offersfilters.subTypes.length > 0) {
        //     filtered = filtered.filter((product) =>
        //         offersfilters.subTypes.includes(product.sub_type_id)
        //     );
        // }
        if (offersfilters.price[0] !== 0 || offersfilters.price[1] !== 6000) {
            filtered = filtered.filter(
                (product) =>
                    product.sale_price_after_discount >= offersfilters.price[0] &&
                    product.sale_price_after_discount <= offersfilters.price[1]
            );
        }
        // if (offersfilters.rating !== null) {
        //     filtered = filtered.filter((product) => {
        //         const productRating = product.rating || 0;
        //         return Math.round(productRating) >= offersfilters.rating;
        //     });
        // }
        setFilteredProducts(filtered);
    }, [offersfilters, offers]);
